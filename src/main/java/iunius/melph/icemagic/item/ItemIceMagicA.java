...
public class ItemIceMagicA extends ItemIceMagic {

	public ItemIceMagicA() {
		super();
	}
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn, EnumHand hand) {
		...
		if (cooldown == 0.0F && hand == EnumHand.MAIN_HAND) {
			...
			playerIn.getCooldownTracker().setCooldown(itemStackIn.getItem(), 40);
			worldIn.playSound((EntityPlayer)null, playerIn.posX, playerIn.posY, playerIn.posZ, SoundEvents.entity_snowball_throw, SoundCategory.NEUTRAL, 0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));

			if (!worldIn.isRemote) {
				EntityIceMagicA entity = new EntityIceMagicA(worldIn, playerIn);
